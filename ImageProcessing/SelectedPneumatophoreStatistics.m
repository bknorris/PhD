    ...
    for k = 1:length(fname)
        ...
        while repeat == 1
            while yes ~= 1
                ...
                %%%%
                disp('Box dimensions:')
                disp(['x: ' sprintf('%.2f',abs(x(2)-x(1))) ' m'])
                disp(['y: ' sprintf('%.2f',abs(y(2)-y(1))) ' m'])
            ...
            D = zeros(v,vv);d = zeros(v,1);dstd = zeros(v,1);
            n = zeros(v,1);
            h = 0.005;H = h*v;
            for i = 1:v
            ...
            disp(['Selected Area: ' num2str(area)])
            ...
            disp(['Std of stem diameter: ' num2str(dstd(idx))])
            if nstem == 0
                Ps = 0;A = 0;
            end
            disp(['a: ' num2str(A)])
        ...
        prompt = 'Save file to folder? [y/n] ';
        str = input(prompt,'s');
        if strcmp(str,'y')
            repeat = 1;
            yes = 0;
            clear H nstem Ps dsc
            vegdat.name(1:length(qname)) = qname;
            vegdat.z = hc;
            vegdat.n = n;
            vegdat.d = d;
            vegdat.a = a;
            vegdat.dstd = dstd;
            vegdat.deltaS = deltaS;
            vegdat.phi = phi;
            vegdat.area = area;
        ...
        clear R r D n a phi X Y deltaS
    end
end