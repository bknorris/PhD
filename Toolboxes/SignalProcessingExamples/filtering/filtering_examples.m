...
xlabel('Normalised frequency, f / f_N','FontSize',[16])
ylabel('Magnitude','FontSize',[16])
 
%design a second order filter using a butterworth design technique
% 2 = 2nd order
% 0.3 = cutoff of normalized frequency
% low = lowpass filter
[b a] = butter(2, 0.3, 'low')
 
%Show the signal flow diagram of the filter/system
%this function available from <span class="skimlinks-unlinked">http://dadorran.wordpress.com</span>
...
figure(10)
hold on
plot([0:1/(num_bins/2 -1):1], abs(H_stopband),'k');
 
%plot filtered signal
x_filtered_stop = filtfilt(b_stop,a_stop,x);
figure(4);
plot(x_filtered_stop,'k')
