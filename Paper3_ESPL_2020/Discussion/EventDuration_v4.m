...
fid = fopen([rdir 'AutoEventDrunfile_v4.txt']);
...
for i = 1:2
    ...
    for ii = 1:length(folders)
        ...
        for j = 1:length(vpid)
            ...
            %We're really just interested in Hs, taub, ubr, eps
            t3 = tic;
            H10 = runningmean(H10,10*60*20); %20-min running mean
            tb10 = runningmean(tb10,10*60*20);
            Om10 = runningmean(Om10,10*60*20);
            ep10 = runningmean(ep10,10*60*20);
            bds = runningmean(bds,10*60*20);
            fprintf('Running means completed in %0.2f minutes\n',toc(t3)/60)
            ...
            bdmean = NaN(maxNumEvnt,1); %rate of change in bottom distance
            ...
            for k = 1:maxNumEvnt
                ...
                bdmean(k) = nanmean(bds(gid)); %mean bed level during event
            ...
            eventd.(dfn{rvp(vpid(j))}).wave.bdmean = wvnbd; %rate of net change in bed level in event
            ...
            eventd.(dfn{rvp(vpid(j))}).ig.bdmean = ignbd;
        ...
        save([npath 'Wavelet\' folders{ii} '\' 'waveIGevents_v4'],'eventd','-v7.3')
    end
end