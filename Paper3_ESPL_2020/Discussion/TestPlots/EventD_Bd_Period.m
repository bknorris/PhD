...
for i = 1:3
    ...
    ylabel(sp(1),'Net Elevation Change')
    ylabel(sp(2),'Net Elevation Change')
    ylabel(cb(1),'Period (sec)')
    ylabel(cb(2),'Period (min)')
    set(sp(1),'xlim',[10^-2 1],'ylim',[-10E-4 10E-4],...
        'position',[0.11 0.16 0.25 0.75],...
        'xscale','log')
    set(sp(2),'xlim',[10^-2 100],'ylim',[-1.5E-3 1.5E-3],...
        'position',[0.6 0.16 0.25 0.75],...
        'xscale','log')
    prettyfigures('text',12,'labels',13,'box',1)
%     export_fig([sfdir expname '_' fn{i} '_evlBDprd'],'-png')
end
