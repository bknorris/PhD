...
rdir = 'g:\GradSchool\DataAnalysis\Paper3\Analysis\';
...
KCexp = log(nfCd./0.47)./-0.052;
ADexp = log(-0.365./(0.261-mlCd))./12.7;
%convert KC's to SD's using Sumer's eq. 9
mlSD = 1.3*(1-exp(-0.03.*(mlKC-6)));
SDexp = 1.3*(1-exp(-0.03.*(KCexp-6)));
ourSD = 1.3*(1-exp(-0.03.*(KC-6)));
xs = [real(ADexp); rad; nfad(KCexp>6)];
ys = [mlSD; SD(:,1); SDexp(KCexp>6)];
...
leg = legend([pi; sf],{'Fringe';'Forest';'Sumer et al. (1992)'});
set(gca,...
    'xlim',[4 150],...
    'ylim',[0.005 2])
sp(2) = subplot(122);
hold on
%Plot datasets
pl(1) = plot(real(ADexp),mlSD,'x',...
    'markersize',6,...
    'color','k'); %ML2004
pl(2) = plot(nfad(KCexp>6),SDexp(KCexp>6),'o',...
    'markersize',4',...
    'linewidth',1,...
    'markerfacecolor','w',...
    'color','k'); %NF1999
pl(3) = plot(rad,SD(:,1),'dk',...
    'markersize',5,...
    'markerfacecolor','k',...
    'color','k');
%do some exp fitting, y = a(aD+b)^n
%parameters from Ezyfit
%first, remove negative values (log doesn't play nicely!)
xs(ys<=0) = [];
ys(ys<=0) = [];
a = 0.0488;
b = 0.19357;
n = -1.8878;
xval = linspace(0,1.2,length(xs(xs>0)));
yval = a*(xval+b).^n;
bfl = plot(xval,yval,'-k','linewidth',1.5);
set(gca,...
    'xscale','log',...
    'yscale','log',...
    'xlim',[10^-2 1],...
    'ylim',[0.005 2])
bfltxt = sprintf('S/D = %0.2f*(aD+%0.2f)^{%0.2f}',a,b,n);
leg2 = legend([pl bfl],{'Mendez & Losada (2004)';'Nepf (1999)';'This study';bfltxt});
%Global Adjustments
set(sp(1),'position',[0.12 0.15 0.35 0.8])
set(sp(2),'position',[0.58 0.15 0.35 0.8])
set(leg,'position',[0.33 0.205 0.05 0.05])
set(leg2,'position',[0.7 0.23 0.05 0.05])
xlabel(sp(1),'KC [-]')
ylabel(sp(1),'S/D [-]')
xlabel(sp(2),'aD [-]')
ylabel(sp(2),'S/D [-]')
...
export_fig([sfdir 'KCSD_aDSD'],'-pdf','-nocrop')
