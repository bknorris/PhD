    ...
    for ii = 1:length(folders)
        ...
        for j = 1:length(vpid)
            ...
            for jj = 1:length(ind)
                ...
                u(zid) = [];
                if rhab(vpid(j)) > 0.1 || rfd50(vpid(j)) %e.g., instruments that can't see the bed
                    znot = rd50(vpid(j));
                    ...
                    if znot > 0.1 || znot < 1E-6 %discard z0 outside expected limits
                        znot = NaN;
                    end
                end
                ...
                phi(jj) = uwstat.phir;
                z0(jj) = znot;
                rsqbins(jj) = idd(maxrsq); %number of lower bins included for reporting in the paper
                z(jj,:) = vph-rb;
            ...
            z = nanmean(z);
            z(z<0.005) = NaN;
            if nanmean(z0) > 1E-2 %some z0 are too big, catch these instances
                z0 = min(z0);
                if z0 > 1E-3 || z0 < 1E-6
                    z0 = rd50(vpid(j));
                    disp('z0 is not in correct range, using d50 for z0')
                end
            else
                z0 = nanmean(z0);
                if isnan(z0)
            ...
            elseif rvegd(vpid(j)) == 0
                tauc = (rho.*((nanmedian(umag,2)*0.4)./(log(nanmedian(z)/z0))).^2)';
            end
            zuv = z;
            ks = z0*30;
