...
files = {'5March2015_Vels.mat';'11March2015_Vels.mat'};
RS = struct();
days = {'f2f2';'f2f3'};
angle = 5; %rotation angle for testing limitations
orienttest = 0;
cutoff = [3.82 2.5 2;3.7 2.67 2];
tic
for i = 1:2
    disp(['Loading ' files{i}])
    dat = load([datdir files{i}]);
    fn = fieldnames(dat);
    if length(fn) == 3
        g = 1:3;
    else
    ...
    for ii = g
        ...
        %loop in time
        fs = 50;
        avt = fs*30; %30 second step
        nwin = fs*300; %5 minute window (25% overlap)
        swin = fs*30; %30 second averaging window (to smooth spectra)
        nsamp = length(time);
        ind = [1 avt:avt:nsamp];
        UW = zeros(1,35);
        VW = zeros(1,35);
...
savedatdir = 'd:\Projects\Mekong_W2015\DataAnalysis\Paper3\RS\Two\';
fname = 'RStress_10min';
if orienttest
    fname = [fname '+_-5deg'];
end
