...
for i = 1:2
    ...
    for ii = 1:length(folders)
        ...
        set(sp(1),'xticklabel',[],'position',[0.11 0.57 0.8 0.353],...
            'ylim',[-10 10])
        set(sp(2),'position',[0.11 0.13 0.8 0.35])
        datetick('x','HH:MM','keepticks','keeplimits')
        title(sp(1),['\bf' rdate{inid(j)}])
        ylabel(sp(1),'BLE [mm]')
        ylabel(sp(2),'epsilon [W/m^2]')
%         ylabel(sp(2),'k [m^2/s^2]')
        xlabel(sp(2),'Time of Day [HH:MM]')
        leg = legend(p,{'n = 0';'n = 102';'n = 37'},'position',[0.78 0.8 0.15 0.15]);
...
%         export_fig([sfpath rdate{inid(1)} '_bedleveldisspts'],'-pdf')
    end
end