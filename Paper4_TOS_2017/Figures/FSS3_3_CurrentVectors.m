...
symb = {'o';'^';'s';'d';'p';'h'};
scale = 0.01;
start = datenum(2015,03,10,15,38,00);
stop = datenum(2015,03,11,07,20,00);
count = 1;
...
files = dir([pw '*100315.mat']);files = {files.name};
...
xlabel('lon')
ylabel('lat')
title('Current Vectors, FSS3-3')
export_fig([savefigdir 'FSS3_3_allCurvec'],'-pdf','-nocrop')
figure(2)
xlabel('Time on 10/03/2015')
ylabel('dbar')
title('Pressure Records, FSS3-3 Multiple Instruments')
legend(pp,names)
export_fig([savefigdir 'FSS3_3_allPres'],'-pdf','-nocrop')
figure(3)
xlabel('lon')
ylabel('lat')
title('Progressive Vectors, FSS3-3 Multiple Instruments')
legend(pd,names,'location','southeast')
export_fig([savefigdir 'FSS3_3_allPVDs'],'-pdf','-nocrop')
