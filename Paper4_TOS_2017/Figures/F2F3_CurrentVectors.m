...
pw = 'D:\Projects\Mekong_W2015\Data\Aquadopp\F2F3\';
files = dir([pw '*12March2015.mat']);files = {files.name};
names = {'5116';'5117';'HR3';'V5109';'VC01';'VC101'};
symb = {'o';'^';'s';'d';'p';'h'};
scale = 0.01;
start = datenum(2015,03,11,15,55,00);
stop = datenum(2015,03,12,08,12,00);
count = 1;
...
clear aqdp
%Load the Vector
pw = 'D:\Projects\Mekong_W2015\Data\Vector\F2F3\';
files = dir([pw '*110315.mat']);files = {files.name};
...
xlabel('lon')
ylabel('lat')
title('Current Vectors, F2F-3')
export_fig([savefigdir 'F2F3_allCurvec'],'-pdf','-nocrop')
figure(2)
xlabel('Time on 11/03/2015')
ylabel('dbar')
title('Pressure Records, F2F-3 Multiple Instruments')
legend(pp,names);
export_fig([savefigdir 'F2F3_allPres'],'-pdf','-nocrop')
figure(3)
xlabel('lon')
ylabel('lat')
title('Progressive Vectors, F2F-3 Multiple Instruments')
legend(pd,names,'location','southeast')
export_fig([savefigdir 'F2F3_allPVDs'],'-pdf','-nocrop')
