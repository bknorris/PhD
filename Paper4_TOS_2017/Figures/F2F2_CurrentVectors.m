%Figure out the current rotations for the F2F2 (2015) experiment

close all
clear

pw = 'D:\Projects\Mekong_W2015\Data\Aquadopp\F2F2\';
files = dir([pw '*2015.mat']);files = {files.name};
names = {'5116';'5117';'HR3';'V5109';'VC01';'VC101'};
symb = {'o';'^';'s';'d';'p';'h'};
scale = 0.01;
start = datenum(2015,03,06,13,10,00);
stop = datenum(2015,03,07,06,22,00);
count = 1;
...
clear aqdp
%Load the Vector
pw = 'D:\Projects\Mekong_W2015\Data\Vector\F2F2\';
files = dir([pw '*050315.mat']);files = {files.name};
...
xlabel('lon')
ylabel('lat')
title('Current Vectors, F2F-2')
export_fig([savefigdir 'F2F2_allCurvec'],'-pdf','-nocrop')
figure(2)
xlabel('Time on 06/03/2015')
ylabel('dbar')
title('Pressure Records, F2F-2 Multiple Instruments')
legend(pp,names)
export_fig([savefigdir 'F2F2_allPres'],'-pdf','-nocrop')
figure(3)
xlabel('lon')
ylabel('lat')
title('Progressive Vectors, F2F-2 Multiple Instruments')
legend(pd,names)
export_fig([savefigdir 'F2F2_allPVDs'],'-pdf','-nocrop')
