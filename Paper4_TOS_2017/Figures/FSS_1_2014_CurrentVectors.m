%Figure out the current rotations for the FSS-1 (2014) experiment

close all
clear

pw = 'D:\Projects\Mekong_F2014\Data\Aquadopp\FSS.1\';
files = {'HR5116.mat';'HR3.mat';'HR4.mat'};
names = {'5116';'HR3';'HR4'};
symb = {'o';'^';'s';'d';'p';'h'};
scale = 0.01;
start = datenum(2014,09,25,12,32,00);
stop = datenum(2014,09,25,14,02,00);
count = 1;
...
xlabel('lon')
ylabel('lat')
title('Current Vectors, FSS-1')
export_fig([savefigdir 'FSS_1_allCurvec'],'-pdf','-nocrop')
figure(2)
set(gca,'YLim',[0 2])
xlabel('Time on 27/09/2014')
ylabel('dbar')
title('Pressure Records, FSS-1 Multiple Instruments')
legend(pp,names)
export_fig([savefigdir 'FSS_1_allPres'],'-pdf','-nocrop')
figure(3)
xlabel('lon')
ylabel('lat')
title('Progressive Vectors, FSS-1 Multiple Instruments')
legend(pd,names,'location','southwest')
export_fig([savefigdir 'FSS_1_allPVDs'],'-pdf','-nocrop')
