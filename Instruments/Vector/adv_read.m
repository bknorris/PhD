function adv = adv_read(filename,metadata)
...
while (~strncmp(str,'Hardware configuration',22));
     ...
     elseif (strncmp(str,'Software version',15))
         Instmeta.softwarever = num2str(str(39:42));
...
fclose(hdr);
adv.metadata = Instmeta; %write metadata

%read the .sen file and write to the adv structure
...
while ~feof(sen)
    ...
    adv.heading(count,:) = str2num(X{SENFIELDS.Heading});    
    adv.pitch(count,:) = str2num(X{SENFIELDS.Pitch});
    adv.roll(count,:) = str2num(X{SENFIELDS.Roll});
    adv.batt_voltage(count,:) = str2num(X{SENFIELDS.Battvolt});
    count = count+1;
end
