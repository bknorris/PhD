...
%%%%
clear
maindir = 'd:\Projects\Mekong_F2014\DataAnalysis\Paper1\';
savedatdir = 'd:\Projects\Mekong_F2014\DataAnalysis\Paper1\TKE\WithN\';
dateofexperiment = '2014'; %'2015'                                              %for time adjustment from GMT to ICT or NZT (depends on the year)
...
for k = 1:length(toprocess)
    ...
    window = 60; %1 second averaging interval
    step = 30; %1/10 second step
    fs = 50;
        ...
        for ii = 1:length(ind)                                                  %loop through time, windowed to the settings
            ...
            for j = 1:4                                                         %loop through beams
                ...
                while itt <= maxbin                                             %loop in depth
                    ...
                    %%%CHANGE THIS LINE TO USE THE \ OPERATOR!!!! IT's WAY
                    %%%FASTER!!!!
...
%                     TKE.(dfn{j}).yfit(:,count:count+lags-1) = pv2;
    ...
    file = regexprep(files{toprocess(k)},'.mat','');
    disp(['Saving ' file ' TKE file'])
    fname = [file 'TKE'];
    save([savedatdir fname],'Stat','-v7.3')
    keep k maindir savedatdir dateofexperiment files toprocess
end
