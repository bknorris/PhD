...
%this section calculates the mean of the y values corresponding to each x bin from histc
idx=find(bin>0);
sum=full(sparse(bin(idx),idx*0+1,y(idx)));
sum=[sum;zeros(length(gx)-length(sum),1)*nan];% add zeroes to the end
b=sum./nn; %bin averages

%this section calculates the std of the y values corresponding to each x bin from histc
if nargout>1, n=nn;end
if nargout>2
	sum=full(sparse(bin(idx),idx*0+1,y(idx).^2));
	sum=[sum;zeros(length(gx)-length(sum),1)*nan];	% add zeroes to the end
    s=sqrt(sum./(nn) - b.^2.*nn./(nn));
end