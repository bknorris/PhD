...
% clear %a good idea
Solo = load('c:\Users\bkn5\Projects\Mekong_W2015\Data\RBR\SoloP\FSS3_to_F2F3\Solo81P_120315.mat');
CTD = load('c:\Users\bkn5\Projects\Mekong_W2015\Data\RBR\XR420\RBR_15244.mat');
dname = 'FSS31'; %deployment
name = 'S81P'; %instrument
plotfigs = 0;
plotwpf = 0;
z = 0; %pressure sensor height
intv = 10; %averaging interval (minutes)
start = datenum(2015,03,7,13,36,00);
stop = datenum(2015,03,7,17,08,00);
savedatfile = 1;
...
ind1 = find(Solo.RBR.Datetime >= start & Solo.RBR.Datetime <=stop);
pressure = Solo.RBR.SeaPres(ind1);
...
%length(pres)
temperature = spline(CTD.RBR.Datetime(ind2),temp,Solo.RBR.Datetime(ind1));
%Initialize Global Variables %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
WPF = struct();
fs = str2double(Solo.RBR.Metadata.samprate(1)); %sampling frequency
...
while it < length(idxx)
    ...
    x = (sin(Solo.RBR.Metadata.latitude/57.29578))^2;
