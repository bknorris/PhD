...
%be only for plotting RBR Loggers, since they have a different sample rate
%from the Aquadopps.

clear
filelist = {'Solo80P_070315.mat';'Solo81P_070315.mat';...
    'Solo83P_070315.mat';'RD02_070315.mat'};
name = {'S80P';'S81P';'S83P';'RD02'};
fdir = 'C:\Users\bkn5\Projects\Mekong_W2015\Data\RBR\SoloP\F2F2\';
CTD = load('c:\Users\bkn5\Projects\Mekong_W2015\Data\RBR\XR420\RBR_15244.mat');
dname = 'F2F2'; %deployment
plotfigs = 0;
savefigs = 0;
z = 0; %pressure sensor height
...
for kk = 1:length(filelist)
    ...
    %since the RBRs recorded over multiple cycles, provide limits for when the
    %instrument was underwater.
    ind1 = find(RBR.Datetime >= start & RBR.Datetime <=stop);
    datetime = RBR.Datetime(ind1);
    pressure = RBR.SeaPres(ind1);
    ind2 = find(CTD.RBR.Datetime >= start & CTD.RBR.Datetime <=stop);
    temp = CTD.RBR.Temperature(ind2);
    %CTD records at 0.25Hz; spline to create a ts where length(temp) =
    %length(pres)
    temperature = spline(CTD.RBR.Datetime(ind2),temp,RBR.Datetime(ind1));
    %Initialize Global Variables %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    WPF = struct();
    fs = str2double(RBR.Metadata.samprate(1)); %sampling frequency
