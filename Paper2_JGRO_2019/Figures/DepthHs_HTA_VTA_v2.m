...
VTA.fr.h = V4.h;
VTA.fr.tr = fastsmooth(V4.T,60,1,1); %odd behavior in Tr, smooth for display
VTA.fr.Hs = V4.Hs;
%times
...
pp(1) = plot(HTA.m.time,HTA.m.tr,'-','LineWidth',1.5,...
    'Color','k');hold on
...
p = patch([t2 e2 e2 t2],[0 0 6 6],[.9 .9 .9]);set(p,'EdgeColor','none')
p = patch([t3 e3 e3 t3],[0 0 6 6],[.9 .9 .9]);set(p,'EdgeColor','none')
pp(1) = plot(VTA.m.time,VTA.m.tr,'-','LineWidth',1.5,...
    'Color','k');hold on
...
set(sp(2),'position',[0.55 0.7 0.42 0.22])
set(sp(3),'position',[0.09 0.42 0.42 0.22])
set(sp(4),'position',[0.55 0.42 0.42 0.22])
set(sp(5),'position',[0.09 0.14 0.42 0.22])
set(sp(6),'position',[0.55 0.14 0.42 0.22])
prettyfigures('text',13,'labels',14,'box',1)
set(leg,'position',[0.6 0.27 0.05 0.05],'box','off')
% export_fig('c:\Users\Bnorr\Documents\GradSchool\Papers\JGR_2018_Norris\Figures\Draft2_Figures\Versions\V3\Figure4','-pdf','-nocrop')
