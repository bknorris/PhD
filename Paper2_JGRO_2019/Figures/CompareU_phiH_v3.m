...
chp = mydata;clear mydata
load('D:\Mekong_W2015\DataAnalysis\Paper2\RS_estimates\NormFrictionU_v2.mat')
usd = mydata;clear mydata
phiHhc = NaN(3,4);
phsd = NaN(3,4);
uwratio = NaN(3,4);
uwsd = NaN(3,4);
...
fn = fieldnames(chp);
for i = 1:4
    dfn = fieldnames(chp.(fn{i}));
    if i < 4
        for ii = 1:3
            phiHhc(ii,i) = nanmean(chp.(fn{i}).(dfn{ii}).Hratio.*chp.(fn{i}).vpro1.phi);
            phsd(ii,i) = nanstd(chp.(fn{i}).(dfn{ii}).Hratio.*chp.(fn{i}).vpro1.phi);
            uwratio(ii,i) = nanmean(chp.(fn{i}).(dfn{ii}).Uwratio);
            uwsd(ii,i) = nanstd(chp.(fn{i}).(dfn{ii}).Uwratio);
            ucratio(ii,i) = nanmean(chp.(fn{i}).(dfn{ii}).Ucratio);
            ucsd(ii,i) = nanstd(chp.(fn{i}).(dfn{ii}).Ucratio)/2;
        end
    else
        phiHhc(1,i) = nanmean(chp.(fn{i}).Hratio.*chp.(fn{i}).phi);
        phsd(1,i) = nanstd(chp.(fn{i}).Hratio.*chp.(fn{i}).phi);
        uwratio(1,i) = nanmean(chp.(fn{i}).Uwratio);
        uwsd(1,i) = nanstd(chp.(fn{i}).Uwratio);
        ucratio(1,i) = nanmean(chp.(fn{i}).Ucratio);
        ucsd(1,i) = nanstd(chp.(fn{i}).Ucratio)/2;
    end
end
for i = 1:4
    dfn = fieldnames(usd.(fn{i}));
    for ii = 1:3
        usratio(ii,i) = nanmean(usd.(fn{i}).(dfn{ii}).usnorm);
        ussd(ii,i) = nanstd(usd.(fn{i}).(dfn{ii}).usnorm);
    end
end
phiHhc(2,4) = 2.8E-3*nanmean(chp.day4.Hratio);
phiHhc(3,4) = 0;
...
fid = fopen('c:\Users\Bnorr\Documents\GradSchool\DataAnalysis\Paper2\StudyComparison\UstarCompare_v2.csv');
hdr = textscan(fgetl(fid),'%s','delimiter',',');hdr = hdr{:}';
cdata2 = textscan(fid,'%s%s%n%n','delimiter',',');
cdata2 = cell2struct(cdata2,hdr,2);
...
cmap = cmap./255;
%combine the data for BF line - Erik's data
id = strcmp(cdata2.Study,'Horstman2017');
xs = cdata2.PhiHhc(id);
ys = cdata2.usuhc(id);
...
%other data
id1 = strcmp(cdata2.Study,'Liu2010');
id2 = strcmp(cdata2.Study,'Lowe2005');
xs = [reshape(phiHhc,12,1); cdata2.PhiHhc(id1); cdata2.PhiHhc(id2)];
ys = [reshape(usratio,12,1); cdata2.usuhc(id1); cdata2.usuhc(id2)];
ys(xs==0) = [];xs(xs==0)=[];
pv = polyfit(log(xs),log(ys),1);
y_hat=exp(pv(1)*log(xs)+pv(2));
bf2 = loglog(xs,y_hat,'-k','linewidth',1.5);
%Horstman2017 - low density
id = strcmp(cdata2.CanopyType,'Pneum');
xs = cdata2.PhiHhc(id);
ys = cdata2.usuhc(id);
sp1 = plot(xs,ys,'^',...
    'color',cmap(1,:),...
    ...
    'markersize',7);
%Horstman2017 - dowvar
id = strcmp(cdata2.CanopyType,'DowVar');
xs = cdata2.PhiHhc(id);
ys = cdata2.usuhc(id);
sp2 = plot(xs,ys,'s',...
    'color',cmap(2,:),...
    ...
    'markersize',7);
%Liu 2010
id = strcmp(cdata2.Study,'Liu2010');
xs = cdata2.PhiHhc(id);
ys = cdata2.usuhc(id);
sp3 = plot(xs,ys,'+',...
    'color',cmap(4,:),...
    ...
    'markersize',7);
%Lowe 2005
id = strcmp(cdata2.Study,'Lowe2005');
xs = cdata2.PhiHhc(id);
ys = cdata2.usuhc(id);
sp4 = plot(xs,ys,'o',...
    'color',cmap(6,:),...
...
xs = reshape(phiHhc,12,1);
ys = reshape(usratio,12,1);
xerr = reshape(phsd,12,1);
yerr = reshape(ussd,12,1);
eb1 = ploterr(xs,ys,xerr,yerr,'.','logxy','abshhxy',0.07);
...
legtxt = {'This Study';...
    'HM2017 - Pneum';'HM2017 - Dowels';'Liu2010';...
    'LW2005 - Uni'};
leg = legend([sp5(1),sp1(1),sp2(1),sp3(1),...
    sp4(1)],...
    legtxt,'fontsize',12,'linewidth',1.5);
set(leg,'position',[0.17 0.14 0.6 0.15])
xlabel('\it\phi(h/h_c) [-]')
tl = ylabel('$\sqrt{-\overline{u''w''}}_{max}\bigg/U_{\infty} \quad [-]$');set(tl,'Interpreter','latex','fontname','arial')
set(gca,'xlim',[10^-4 10^0],...
    'ylim',[0.01 2],...
    'position',[0.16 0.13 0.8 0.8])
hold off
% export_fig([figdir 'Ustar_Uhc'],'-pdf')
break
%Plot the figure
f2 = figure(2);
...
yerr(isnan(yerr)) = [];
eb1 = ploterr(xs,ys,xerr,yerr,'.','logxy','abshhxy',0.1);
set(eb1,'linewidth',1.5,'color','k')
sp2 = plot(xs,ys,'d',...
    'color',cmap(8,:),...
    'linewidth',1.5,...
