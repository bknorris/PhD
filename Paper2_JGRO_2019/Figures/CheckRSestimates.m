...
%velocities. 
clear
load('D:\Projects\Mekong_W2015\DataAnalysis\Paper2\RS_estimates\RStress_10min_v5_1deg_pitch.mat')
pitchp = RS;
load('D:\Projects\Mekong_W2015\DataAnalysis\Paper2\RS_estimates\RStress_10min_v5_-1deg_pitch.mat')
pitchn = RS;
load('D:\Projects\Mekong_W2015\DataAnalysis\Paper2\RS_estimates\RStress_10min_v5_1deg_roll.mat')
rollp = RS;
load('D:\Projects\Mekong_W2015\DataAnalysis\Paper2\RS_estimates\RStress_10min_v5_-1deg_roll.mat')
rolln = RS;
...
for i = 1:4
    ...
    for j = 1:3
        ...
        uwci = zeros(35,5);
        vwci = zeros(35,5);
        ppuw = pitchp.(dn{i}).(fn{j}).uw;
        ppvw = pitchp.(dn{i}).(fn{j}).vw;
        pnuw = pitchn.(dn{i}).(fn{j}).uw;
        pnvw = pitchn.(dn{i}).(fn{j}).vw;
        rpuw = rollp.(dn{i}).(fn{j}).uw;
        rpvw = rollp.(dn{i}).(fn{j}).vw;
        rnuw = rolln.(dn{i}).(fn{j}).uw;
        rnvw = rolln.(dn{i}).(fn{j}).vw;
        uw = RS.(dn{i}).(fn{j}).uw;
        vw = RS.(dn{i}).(fn{j}).vw;
        %UW
