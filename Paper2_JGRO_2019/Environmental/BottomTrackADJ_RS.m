...
load('D:\Projects\Mekong_W2015\DataAnalysis\Paper2\OldFiles\BDtrack_RS.mat')
load('d:\Projects\Mekong_W2015\DataAnalysis\Paper2\RS_estimates\RStress_10min_v5')
date = {'day1';'day4'};
...
for k = 1
    ...
    for i = 1:3
        ...
        for j = 1:length(bh);
            ...
            RS.(date{k}).(dn{i}).uwint(j,id:end) = NaN;
            RS.(date{k}).(dn{i}).vwint(j,id:end) = NaN;
            RS.(date{k}).(dn{i}).ku0(j,id:end) = NaN;
            RS.(date{k}).(dn{i}).kv0(j,id:end) = NaN;
            RS.(date{k}).(dn{i}).kc(j,id:end) = NaN;
        end
    end
end
dirname = 'd:\Projects\Mekong_W2015\DataAnalysis\Paper2\RS_estimates\';
save([dirname 'RStress_10min_v5_bdadj'],'RS','-v7.3')